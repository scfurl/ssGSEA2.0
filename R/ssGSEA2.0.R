...
# }
  ...
  #source(paste(this.file.dir, 'gct-io.R', sep='/'))
...
ssGSEA2 <- function (
                     ...
                     par=F,
                     use.cores=1,
                     export.signat.gct=F, ## if TRUE gct files with expression values for each signature will be generated
                     param.file=T,
                     log.file='run.log') {
    ## #######################################################################
    ## single sample GSEA
    ...
    if(export.signat.gct & combine.mode!="combine.off"){stop("No current support for overlap gct export and any other combine mode than 'combine.off'")}
